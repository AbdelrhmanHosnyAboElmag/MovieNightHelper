...
class GetSimilarMovieUseCase @Inject constructor(private val MovieRepositoryImpl: MovieRepository) {
    operator fun invoke(movieId:Int): Flow<DataResult<SimilarMovie>> = flow {
        try {
            emit(DataResult.Loading())
            val repo = MovieRepositoryImpl.getMovieSimilar(movieId)
