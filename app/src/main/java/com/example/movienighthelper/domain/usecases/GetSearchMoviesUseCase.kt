...
class GetSearchMoviesUseCase @Inject constructor(private val MovieRepositoryImpl: MovieRepository) {
    operator fun invoke(query: String): Flow<DataResult<PopularMovie>> = flow {
        try {
            emit(DataResult.Loading())
            val repo = MovieRepositoryImpl.getSearchMovies(query)
